// main.cpp — TaskForge demonstration driver
//
// Assumed public interfaces (adjust names/signatures to match your actual headers):
//   ToDoTask(const string& name, const string& description, State* state, int priority = 0, const string& dueDate = "")
//   ProductionGroup(string name, string description)
//   ProductionGroup::add(ToDoItem*) / ::remove(ToDoItem*)   // remove() does NOT delete
//   ToDoTask::start()/pause()/cancel()/complete()/execute() // delegate to State internally
//   ToDoItem::setEstimatedCost(float)
//   PriorityDecorator(ToDoItem* i, int priority)
//   CostAdjustmentDecorator(ToDoItem* i, float adjustmentAmount)
//   ToDoItem::createIterator(string type) -> ItemIterator*  // "standard" | "cost"
//
//   State is NOT a singleton: each ToDoTask below gets its own freshly constructed
//   Planning instance. This means ToDoTask.cpp must own and delete its state — in
//   ~ToDoTask() and again inside setState() (delete the old state before assigning
//   the new one) — otherwise every lifecycle transition leaks. Main cannot enforce
//   this from the outside; verify it in ToDoTask.cpp with Valgrind.
//
// ============================== OWNERSHIP MAP ==============================
// shootDay
//   -> scene14
//        -> cameraSetup                              (leaf, owned by scene14)
//        -> weatherContingency                        (leaf, owned by scene14)
//        (stuntRehearsal is REMOVED from here mid-program — see Scenario 2)
//   -> soundDept
//        -> boomCheck                                 (leaf, owned by soundDept)
//   -> catering                                        (leaf, owned by shootDay)
//   -> rescheduled                                     (added mid-program)
//        -> urgentStuntWithCost                        (CostAdjustmentDecorator)
//             -> urgentStunt                           (PriorityDecorator, owned by urgentStuntWithCost)
//                  -> stuntRehearsal                   (owned by urgentStunt)
//
// A single `delete shootDay;` at the end walks this entire tree exactly once.
// No pointer above is ever deleted more than once, and every `new` is reachable
// from shootDay by the time cleanup runs.
//
// Iterators (standardIt, costIt, and the ones created inside printAll) are the
// only objects NOT owned by the tree — main creates and deletes each one itself,
// immediately after use, and never lets one outlive the scope that made it.
// ============================================================================
